...
def give_bmi(height: list[int | float],
    ...
    zip() mostly used to iterate though pairs
    elements form two or more iterables together.
    """
    bmis = []
...
def apply_limit(bmi: list[int | float], limit: int) -> list[bool]:
    """
    Apply limits (True if above the limit),
    in case limit is non-posivite number throw error.
    """
    if (abs(limit) != limit):
